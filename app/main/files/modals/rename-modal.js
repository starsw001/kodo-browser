angular.module('web')
  .controller('renameModalCtrl', ['$scope', '$uibModalInstance', '$translate', '$uibModal', 'item', 'isCopy', 'currentInfo', 'moveTo', 'callback', 'QiniuClient', 'Dialog', 'Toast', 'AuditLog',
    function ($scope, $modalInstance, $translate, $modal, item, isCopy, currentInfo, moveTo, callback, QiniuClient, Dialog, Toast, AuditLog) {
      ...
      if (!form.$valid) return;
        ...
        if ($scope.item.itemType === 'folder') {
          ...
          QiniuClient.checkFolderExists(moveTo.regionId, moveTo.bucketName, newPath).then(function (has) {
            if (has) {
          ...
          $scope.isLoading = true;
          QiniuClient.checkFileExists(moveTo.regionId, moveTo.bucketName, newPath).then((exists) => {
            if (exists) {
      ...
      function renameFile(newPath) {
        ...
        Toast.info(onMsg);
        QiniuClient.moveOrCopyFile(currentInfo.regionId, currentInfo.bucketName, item.path, newPath, isCopy).then(() => {
